...
data %>% group_by(crime) %>% summarise(mean = mean(response))
...
The average callback rate for people who committed crimes is 10.94% and is 12.51% for individuals who did not commit crimes for all types of applications across both time periods. The difference is statistically significant at a 95% confidence level.
...
stargazer(reg1, type = "latex", title = "Call Back Rates With Fixed Effects (Box Only, Both Periods)",
          covariate.labels = c("White", "GED",
                               "Employment Gap", "Chain FE", "Center FE"), 
...
The coefficient on white further decreases in magnitude even without the fixed effects (Table 3). In the absence of the fixed effects, the white coefficient changes from $-0.001140$ to $-0.001117$. This is a clear example of the omitted variable bias influencing the white coefficient.
...
The coefficient on crimbox in this model is not statistically significant. The interaction of crimbox and white is only statistically significant at the 90% level. The interaction coefficient signifies that being white and submitting for a position where the box is present slightly diminishes the likelihood ($-0.0296$) of a callback. This is simultaneously offset by the crimbox coefficient ($0.0158$) despite the fact that it's not statistically significant.
...
reg5 <- data %>% filter(., crimbox==1) %>% 
  lm(response~white + crime + ged + empgap + chain_id + center, data = .)
...
stargazer(reg5, type = "latex", title = "Call Backs Rates (Box Only, Both Periods)",
          covariate.labels = c("White", "Crime",
                               "GED","Employment Gap","Chain FE", "Center FE"), 
          digits = 6, single.row = TRUE,
          se = list(robust_se))
```

In this model, subsetting the data for only applications that feature the box requires dropping the box and box * white interaction variables. Given that, being white does limit the response rate ($-0.0014$) to a certain degree and so does having previously committed a crime ($-0.0497$). The response rate declines in greater magnitude given a crime as opposed to being white.
